<?php
	...
	**/
	 ...
	 * 	Salida:
	 * 		$sincro = booleano.
	 * 		TRUE si es fue exitosa, FALSE si hubo un problema.
	 * 	Notas:
	 * 	Actualizar:
	 * 		Agregar logging.
	 */
	function comparar_y_sincronizar_orig() {
		$dir = new Directorio();
		$lista_sincro = array( );
		$sicro = FALSE;
		$patron = array();
		...
		for($i=0; $i<3; $i++) {
			echo "--- {$patron[$i]} ---";
			echo "<br>";
			$lista_sincro = $dir->lista_sincronizacion(DIR_ORIGEN,DIR_DESTINO,$patron[$i]);	
			$dir->sincronizar(DIR_ORIGEN,DIR_DESTINO,$patron[$i]);
